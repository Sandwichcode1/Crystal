...
class AutomodDisable(commands.Cog):
    ...
    @app_commands.command(name="automod_disable", description="Disable AutoMod for this server")
    async def automod_disable(self, interaction: discord.Interaction):
        ...
        if data.get(guild_id, {}).get("enabled") is False:
            msg = "⚠️ AutoMod is already disabled."
        else:
            data[guild_id] = {"enabled": False}
            self._save_status(data)
            msg = "❌ AutoMod has been disabled for this server."

        await interaction.response.send_message(msg, ephemeral=True)

async def setup(bot):
    await bot.add_cog(AutomodDisable(bot))
